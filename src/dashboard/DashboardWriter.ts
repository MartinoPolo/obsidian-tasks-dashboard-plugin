...
export class DashboardWriter {
	...
	async addIssueToDashboard(dashboard: DashboardConfig, issue: Issue): Promise<void> {
		...
		let file = this.app.vault.getAbstractFileByPath(dashboardPath) as TFile | null;
		if (file === null) {
	...
	async moveIssueToArchive(dashboard: DashboardConfig, issueId: string): Promise<void> {
		const dashboardPath = `${dashboard.rootPath}/Dashboard.md`;
		const file = this.app.vault.getAbstractFileByPath(dashboardPath) as TFile | null;
		if (file === null) {
			return;
		}
		let content = await this.app.vault.read(file);
	...
	private buildIssueEntry(issue: Issue, _dashboard: DashboardConfig): string {
