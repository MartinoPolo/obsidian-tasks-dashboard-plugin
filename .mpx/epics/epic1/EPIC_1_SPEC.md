...
- [x] Branch badge: green when active on remote, light green when local only, amber when remote deleted (upstream config detected), strikethrough when fully deleted, gray when unknown
- [x] Branch state determined by checking remote ref first, then local ref; upstream tracking config (`branch.<name>.remote`) used to detect remote-deleted state
