...
- **Epic 1** — Core plugin: dashboard structure, issue lifecycle, GitHub integration, worktree integration, color system, project folders, settings, git status badges, priority toggle, code quality. 29 phases. See [epics/epic1/](epics/epic1/).
- **Epic 2** — Svelte Migration: migrated all UI from vanilla DOM manipulation to Svelte 5 components. See [epics/epic2-svelte-migration/](epics/epic2-svelte-migration/).
- **Epic 3** — Dashboard UX, Sync & Polish: badge icons, base branch sync detection, sync action, badge contrast, color picker, closed issue appearance, info panel, toolbar reorder, prune worktrees, issue creation safety, refactoring, local git branch sync, cleanup. 14 phases. See [epics/epic3/](epics/epic3/).
...
**Phase 11: Local Git Detection Infrastructure** — Complete. Added `runGitCommandAsync` async spawn wrapper, `createFetchCoordinator` for fetch deduplication per repo root, `getBehindCount` via `git rev-list`, `detectMergeConflicts` via `git merge-tree --write-tree`. Replaced GitHub API calls with local git operations.

**Phase 12: Direct Git Sync (Single Branch)** — Complete. On sync badge click: `isWorktreeDirty` check with DirtyWorktreeModal fallback, `detectMergeConflicts` with Claude Code terminal fallback, then `git merge + git push` (async) with SyncProgressModal showing step-by-step feedback.

**Phase 13: Sync All Branches** — Complete. Toolbar "Sync All" rewired: single `git fetch origin` per unique repo root via FetchCoordinator, parallel per-branch sync, unified SyncAllProgressModal with per-branch status lines.

**Phase 14: Cleanup** — Complete. Removed `compareBranches`, `getPullRequestMergeable`, `BranchCompareResult`, `GitHubCompareApiResponse`, `OPEN_PR_STATES` dead code. Updated `sync-constants.ts` with fallback-only clarification.

</details>

## Current Epic

- **Epic 4** — Quick Fixes: color picker reset-to-default, enter-to-confirm rename. See [SPEC.md](SPEC.md).

## Decisions

### Epic 2 (Svelte Migration)

- Separate epic from core plugin work; all UI rewritten from vanilla DOM to Svelte 5 components.

### Epic 3

- "Running dev environments" feature intentionally skipped.
...
- Phases 11-14 extend the phase numbering from 01-10. Detection infrastructure grouped into one phase (tightly coupled). Single-branch sync and Sync All are separate phases (different UI patterns). Cleanup is a separate final phase.
- Local git detection moved outside `if (githubService.isAuthenticated())` block — works without GitHub token.
