...
- [ ] GitHub badges should have a dark and light variant, and those should be automatically switched based on the issue header background and text color selected with a color picker. Currently, whenever we select the color for an issue in the dashboard, it affects the background of the issue card as well as the text color, which is now used for the title and the badges, issue info icon, and all other buttons. I'd also consider the text color to also be the batch border color So that there is always enough contrast between the background issue background and the color in the batch as the background. Feel free to suggest a better system where the badge state is always visible enough in this context.
...
- [ ] There should also be a letter A in the custom color picker. We currently have letter A with the correct color in each square representing a color preset. I'd also like the same letter A to be displayed in the custom color picker with the color of the text, so that we can see how the text color would look like in the issue header if we select that color.
